...
public class SarifFileUploadHtmlGenerator {
  ...
  public static String generateHTML(AnalysisResult result, String serverAddress)
      throws IOException {
    SarifFileUploadHtmlGenerator.serverAddress = serverAddress;

    return html(generateHeader(), generateBody(result)).renderFormatted();
  }

  private static ContainerTag generetaH1Title() {
    return h1("Welcome to MagpieBridge SARIF file upload Page!");
  }
  ...
  private static DomContent generateBody(AnalysisResult result) throws IOException {
    Iterable<Pair<Position, String>> flows = result == null ? null : result.related();
    return body(
        div(
                div(div(
                            generetaH1Title(),
  ...
  private static ContainerTag generateForm() {
    ContainerTag ret =
        form()
            .withMethod("post")
            .withAction("/sarif-file")
            .withId("sarifFileUploadForm")
            .attr("enctype", "multipart/form-data");
    ret.with(label("Select a file with SARIF format:"));
    ret.with(
        input()
            .withType("file")
            .withName("sarif")
            .withId("upload-button")
